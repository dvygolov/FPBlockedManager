...
function buildLandingHtml({ appName, displayName = appName, build, bookmarklet, manifestUrl, screenshotUrl, iconUrl }) {
  ...
  return [
    ...
    "          <p class=\"lead\">FPBlockedManager is a browser-side tool for exporting blocked Facebook user IDs from a Page to TXT and importing the same one-ID-per-line TXT into another Page.</p>",
    "          <div class=\"install\">",
    ...
    "      <div class=\"card\"><strong>Page selector</strong><p>Loads manageable Facebook Pages automatically when the panel opens and keeps work on one selected Page.</p></div>",
    "      <div class=\"card\"><strong>Export blocked</strong><p>Saves blocked user IDs from the selected Page into a plain TXT file, one ID per line.</p></div>",
    "      <div class=\"card\"><strong>Import blocked</strong><p>Reads a TXT file and blocks every user ID line-by-line on the selected Page.</p></div>",
    "      <div class=\"card\"><strong>Compact actions</strong><p>Keeps the panel focused on blocked-user export and import, with status visible in the log.</p></div>",
...
function buildBookmarkletLoader(loaderManifest) {
  ...
  const configJson = JSON.stringify({
    ...
    cacheKey: "fpblockedmanager.loader.cache.v1",
  });
